...
export const BtqView: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Alert Box */}
      <div className="p-6 rounded-3xl bg-gradient-to-r from-rose-500/10 via-amber-500/10 to-emerald-500/10 border border-rose-500/30 text-slate-900 dark:text-white space-y-3 shadow-lg">
        <div className="flex items-center space-x-3">
          <div className="p-2.5 rounded-xl bg-rose-500 text-white font-bold">
            <ShieldAlert className="w-6 h-6" />
          </div>
          <h3 className="font-extrabold text-base">SYARAT MUTLAK KELULUSAN SKRIPSI / TA</h3>
        </div>
        <p className="text-xs sm:text-sm leading-relaxed text-slate-700 dark:text-slate-200">{btqData.alertMessage}</p>
      </div>
        ...
        <div className="glass-card p-6 rounded-3xl border space-y-4">
          ...
          <ul className="space-y-2.5 text-xs text-slate-600 dark:text-slate-300">
            {btqData.passingCriteria.map((req) => (
              <li key={req} className="flex items-start space-x-2.5 p-3 rounded-xl bg-slate-50 dark:bg-slate-900/60 border border-slate-100 dark:border-slate-800">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="glass-card p-6 rounded-3xl border space-y-4 lg:col-span-1">
          <h3 className="font-bold text-base flex items-center space-x-2 text-slate-900 dark:text-white">
          ...
          <div className="max-h-96 overflow-y-auto no-scrollbar space-y-1.5 pr-1">
            {btqData.surahList.map((surah, i) => (
              <div key={surah} className="flex items-center justify-between p-2.5 rounded-xl bg-slate-50 dark:bg-slate-900/60 border border-slate-100 dark:border-slate-800 text-xs">
        ...
        <div className="glass-card p-6 rounded-3xl border space-y-4 lg:col-span-2">
          <h3 className="font-bold text-base flex items-center space-x-2 text-slate-900 dark:text-white">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {btqData.tipsForGradeA.map((tip) => (
              <div key={tip.title} className="p-4 rounded-2xl bg-indigo-50/50 dark:bg-slate-900/60 border border-indigo-100 dark:border-slate-800 space-y-2">
