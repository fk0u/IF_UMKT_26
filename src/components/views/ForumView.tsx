  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-card p-6 rounded-3xl border space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <input
            ...
            className="px-3.5 py-2 text-xs rounded-xl bg-slate-100 dark:bg-slate-800 border border-transparent focus:border-brand-500 focus:outline-none sm:col-span-2"
          />
        </div>
        ...
        <textarea
          ...
          className="w-full px-3.5 py-2 text-xs rounded-xl bg-slate-100 dark:bg-slate-800 border border-transparent focus:border-brand-500 focus:outline-none"
        ></textarea>
        ...
        <div className="flex items-center justify-between pt-1">
          <select
            ...
            className="px-3 py-1.5 text-xs rounded-xl bg-slate-100 dark:bg-slate-800 border border-transparent focus:border-brand-500 focus:outline-none"
          >
      ...
      {forumQuery.isLoading ? (
        <div className="text-center py-8 text-xs text-slate-500">Memuat diskusi forum via TanStack Query...</div>
      ) : (
        <div className="space-y-4">
          {filteredForum.map((item) => (
            <div key={item.id} className="glass-card p-6 rounded-3xl border space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  <img src={item.avatar} alt="avatar" className="w-9 h-9 rounded-full bg-slate-200 dark:bg-slate-800 p-0.5 border" />
                  <div>
                    <h4 className="font-bold text-xs text-slate-900 dark:text-white">{item.author}</h4>
                    <p className="text-[11px] text-slate-400">{item.date}</p>
                  </div>
                </div>
                <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-indigo-100 text-indigo-700 dark:bg-indigo-900/60 dark:text-indigo-300">
                  {item.category}
                </span>
              </div>
              ...
              {item.replies.length > 0 && (
                <div className="space-y-2 pt-2">
                  {item.replies.map((r, idx) => (
                    <div key={idx} className="p-3 rounded-2xl bg-slate-50 dark:bg-slate-900/60 border border-slate-100 dark:border-slate-800/80 text-xs space-y-1 ml-4">
                      <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2 pt-2">
                <input
                  ...
                  className="flex-1 px-3.5 py-1.5 text-xs rounded-xl bg-slate-100 dark:bg-slate-800 border border-transparent focus:border-brand-500 focus:outline-none"
                />
                <button
                  onClick={() => handleReply(item.id)}
                  className="px-3.5 py-1.5 rounded-xl bg-brand-600 text-white font-bold text-xs hover:bg-brand-700 transition"
                >
                  Kirim
                </button>
              </div>
