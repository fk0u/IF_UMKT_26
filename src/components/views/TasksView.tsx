  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-extrabold tracking-tight">Daftar Tugas Perkuliahan</h2>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            Pantau dan kelola pengerjaan tugas kuliah angkatan secara real-time.
          </p>
        </div>
      ...
      {tasksQuery.isLoading ? (
        <div className="text-center py-8 text-xs text-slate-500 font-medium">Memuat data tugas...</div>
      ) : (
        <div className="space-y-3">
          {filteredTasks.map((t) => (
            <div
              key={t.id}
              className="hm-card p-4 sm:p-5 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition hover:border-brand-500/40"
            >
              <div className="space-y-1 flex-1">
                <div className="flex items-center space-x-2">
                  <span className="px-2.5 py-0.5 rounded-full text-[10px] font-mono-tag font-bold bg-brand-100 text-brand-700 dark:bg-brand-900/50 dark:text-brand-300">
                    {t.course}
                  </span>
                  <span
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 font-mono-tag">{t.description || `Deadline: ${t.deadline}`}</p>
              </div>
              ...
              <div className="flex items-center space-x-2 shrink-0">
                <select
                  ...
                  className="px-3 py-1.5 text-xs font-bold rounded-xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-200"
                >
